...
ROLE3_VAL_SIRA = 13
ROLE4_VAL_SIRA = 14
ROLICAM_ANGLE_SIRA = 15
ROLICAM_SPEED_SIRA = 16
ROLICAM_DIM_SIRA = 17
ROLICAM_RESET_SIRA = 18
FILTER_RESET_SIRA = 19
...
rovAdresler = {
    "debug_on": 0x00000000,
    "ust_sag_on_val": 0x00000000,
    "ust_sol_on_val": 0x00000000,
    "ust_sag_arka_val": 0x00000000,
    "ust_sol_arka_val": 0x00000000,
...
class rovAracUart(Serial):
    ...
    def adresAl(self, show=False):
        ...
        for (k, v) in rovAdresler.items():
            print("3", self.readline())
            deger = self.readline().decode().strip()
            print("4")
            if len(deger.split()) == 2:  # Split sonucunda 2 eleman olup olmadığını kontrol ediyoruz
                anahtar, deger_str = deger.split()
                if anahtar in rovAdresler:  # Anahtarın doğru olup olmadığını kontrol ediyoruz
                    rovAdresler[anahtar] = int(deger_str)
                else:
                    print(f"Hatalı anahtar: {anahtar}")
            else:
                print(f"Beklenmeyen formatta veri alındı: {deger}")
            print("5")
        if show:
            for (k, v) in rovAdresler.items():
                print(f"{k}:\t{hex(v)}")
    ...
    def ustSagOnMotorSet(self, val):
        rovDegerlerGelen["ust_sag_on_val"] = constrain(val, ESC_MIN, ESC_MAX)
        self.write(URF_VAL_SIRA.to_bytes(1, "little"))
        self.write(rovDegerlerGelen["ust_sag_on_val"].to_bytes(2, "little"))
        self.read(1)

    def ustSolOnMotorSet(self, val):
        rovDegerlerGelen["ust_sol_on_val"] = constrain(val, ESC_MIN, ESC_MAX)
        self.write(ULF_VAL_SIRA.to_bytes(1, "little"))
        self.write(rovDegerlerGelen["ust_sol_on_val"].to_bytes(2, "little"))
        self.read(1)

    def ustSagArkaMotorSet(self, val):
        rovDegerlerGelen["ust_sag_arka_val"] = constrain(val, ESC_MIN, ESC_MAX)
        self.write(URB_VAL_SIRA.to_bytes(1, "little"))
        self.write(rovDegerlerGelen["ust_sag_arka_val"].to_bytes(2, "little"))
        self.read(1)

    def ustSolArkaMotorSet(self, val):
        rovDegerlerGelen["ust_sol_arka_val"] = constrain(val, ESC_MIN, ESC_MAX)
        self.write(ULB_VAL_SIRA.to_bytes(1, "little"))
        self.write(rovDegerlerGelen["ust_sol_arka_val"].to_bytes(2, "little"))
        self.read(1)
    ...
    def ustMotorlarDurdur(self):
        self.ustSagOnMotorSet(ESC_STOP)
        self.ustSolOnMotorSet(ESC_STOP)
        self.ustSagArkaMotorSet(ESC_STOP)
        self.ustSolArkaMotorSet(ESC_STOP)
    ...
    def yukariGit(self, hiz):
        self.ustSagOnMotorSet(ESC_STOP + constrain(hiz, -500, 500))
        self.ustSolOnMotorSet(ESC_STOP + constrain(hiz, -500, 500))
        self.ustSagArkaMotorSet(ESC_STOP + constrain(hiz, -500, 500))
        self.ustSolArkaMotorSet(ESC_STOP + constrain(hiz, -500, 500))

    def asagiGit(self, hiz):
        self.ustSagOnMotorSet(ESC_STOP - constrain(hiz - 100, 0, 500))
        self.ustSolOnMotorSet(ESC_STOP - constrain(hiz - 100, 0, 500))
        self.ustSagArkaMotorSet(ESC_STOP - constrain(hiz, 0, 500))
        self.ustSolArkaMotorSet(ESC_STOP - constrain(hiz, 0, 500))

    def asagiGitSagSol(self, sag_hiz, sol_hiz):
        self.ustSagOnMotorSet(ESC_STOP - constrain(sag_hiz - 100, 0, 500))
        self.ustSolOnMotorSet(ESC_STOP - constrain(sol_hiz - 100, 0, 500))
        self.ustSagArkaMotorSet(ESC_STOP - constrain(sag_hiz, 0, 500))
        self.ustSolArkaMotorSet(ESC_STOP - constrain(sol_hiz, 0, 500))
...
class rovAracSwd(Swd):
    ...
    def ustSagOnMotorSet(self, val):
        rovDegerlerGelen["ust_sag_on_val"] = constrain(val, ESC_MIN, ESC_MAX)
        self.write_mem(rovAdresler["ust_sag_on_val"], rovDegerlerGelen["ust_sag_on_val"].to_bytes(2, "little"))

    def ustSolOnMotorSet(self, val):
        rovDegerlerGelen["ust_sol_on_val"] = constrain(val, ESC_MIN, ESC_MAX)
        self.write_mem(rovAdresler["ust_sol_on_val"], rovDegerlerGelen["ust_sol_on_val"].to_bytes(2, "little"))

    def ustSagArkaMotorSet(self, val):
        rovDegerlerGelen["ust_sag_arka_val"] = constrain(val, ESC_MIN, ESC_MAX)
        self.write_mem(rovAdresler["ust_sag_arka_val"], rovDegerlerGelen["ust_sag_arka_val"].to_bytes(2, "little"))

    def ustSolArkaMotorSet(self, val):
        rovDegerlerGelen["ust_sol_arka_val"] = constrain(val, ESC_MIN, ESC_MAX)
        self.write_mem(rovAdresler["ust_sol_arka_val"], rovDegerlerGelen["ust_sol_arka_val"].to_bytes(2, "little"))
    ...
    def yukariGit(self, hiz):
        self.ustSagOnMotorSet(ESC_STOP + constrain(hiz - 50, -500, 500))
        self.ustSolOnMotorSet(ESC_STOP + constrain(hiz - 50, -500, 500))
        self.ustSagArkaMotorSet(ESC_STOP + constrain(hiz, -500, 500))
        self.ustSolArkaMotorSet(ESC_STOP + constrain(hiz, -500, 500))
